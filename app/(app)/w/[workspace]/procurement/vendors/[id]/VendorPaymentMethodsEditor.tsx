...
/**
 * VendorPaymentMethodsEditor — single-vendor payment methods
 * editor. Reachable from the vendor detail page and the
 * workspace payment settings page (one row per vendor).
 *
 * Stores the VENDOR's bank / card / check details so the
 * buyer (us) knows where to send payment. Last 4 only — the
 * full account / card number never enters our DB.
 */
export function VendorPaymentMethodsEditor({
  workspaceSlug,
  vendor,
}: {
  workspaceSlug: string;
  vendor: { id: string; name: string; methods: MethodDto[] };
}) {
      ...
      <div className="px-4 py-2 border-b-2 border-ink flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <div className="text-[12px] text-ink-70 mt-0.5">
            How this vendor accepts payment. We use these
            details when paying them (ACH routing, check
            #, or last-4 of their card).
          </div>
        </div>
        <button
          type="button"
          onClick={() => setAdding((v) => !v)}
          className="px-3 py-1.5 bg-ink text-cream text-[10px] font-extrabold uppercase tracking-[0.12em] hover:bg-orange-d"
        >
          {adding ? '✕ Cancel' : '+ Add method'}
        </button>
      </div>
      {vendor.methods.length === 0 ? (
        <div className="text-[12px] text-ink-50 text-center py-6 border-t border-dashed border-line">
          No payment methods on file yet. The vendor portal
          will hide the &ldquo;Pay me on file&rdquo; option
          until at least one method is added.
        </div>
      ) : (
...
function MethodRow({
  ...
  function setDefault() {
    startTransition(async () => {
      const res = await setDefaultVendorPaymentMethodAction({
        workspaceSlug,
        methodId: method.id,
        vendorId,
      });
  ...
  function toggle() {
    startTransition(async () => {
      const res = await toggleVendorPaymentMethodAction({
        workspaceSlug,
        methodId: method.id,
  ...
  function del() {
    ...
    startTransition(async () => {
      const res = await deleteVendorPaymentMethodAction({
        workspaceSlug,
        methodId: method.id,
      });
      if (res.ok) location.reload();
  ...
  return (
    <li className="px-4 py-2 flex items-center gap-2 text-[12px]">
      <span
...
function AddMethodForm({
  ...
  return (
    <div className="px-4 py-3 border-t border-line space-y-2">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        <label className="block">
          ...
          >
            ...
            <option value="CARD">Credit card (3% fee may apply)</option>
            <option value="CHECK">Check by mail</option>
          </select>
        </label>
        <label className="block md:col-span-2">
          <div className="text-[10px] font-mono uppercase tracking-[0.1em] text-ink-50 mb-0.5">Nickname (optional)</div>
          <input
            value={nickname}
      ...
      {methodType === 'CHECK' ? (
        <label className="block">
          <div className="text-[10px] font-mono uppercase tracking-[0.1em] text-ink-50 mb-0.5">Check # or last 4 (for our records) *</div>
          <input
            value={last4}
            onChange={(ev) => setLast4(ev.target.value)}
            maxLength={4}
      ...
      <p className="text-[10px] text-ink-50">
        We never store the full account or card number. Only the last 4 digits + bank/brand metadata. The vendor keeps the rest in their own records.
      </p>
