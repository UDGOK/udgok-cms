  ...
  return (
    <div>
      ...
      {/* Mobile upload sheet (also used on desktop — the
          ...
          The sheet hosts the upload form plus the prominent
      ...
      <BottomSheet open={sheetOpen} onClose={() => setSheetOpen(false)} title="Add project photo" maxHeightClass="max-h-[92vh]">
        <PhotoUploadSheet
          ...
          onUploaded={(newPhotoId) => {
            // After the new row arrives in `photos`, scroll it
            // into view. If the new id isn't found (e.g. the
            // server callback hasn't settled yet), the user
            // can still tap "View photo" once it appears.
            setTimeout(() => {
              if (newPhotoId) {
...
// ============================================================
// UploadProgressCard — large, prominent progress display
// shown in the center of the upload sheet. Same Atelier
// design tokens as the rest of the app — 2px ink border,
// mono labels, paper background — but with a chunkier
// progress bar (h-3) and bigger numbers than the inline
// form version, so the user can see it at a glance.
// ============================================================
...
// ============================================================
// UploadSuccessCard — green confirmation card with the
// uploaded URL and a "View photo" button. The button is
// disabled until the parent has refreshed `photos` and
// we've matched the new row by URL.
// ============================================================
...
// ============================================================
// UploadErrorCard — red error card with the message and a
// "Try again" button. Resets the hook so the user can
// retry the same file.
// ============================================================
...
// ============================================================
// useLatestPhotoIdForUrl — find the ProjectPhoto whose URL
// matches `url`, or null. Re-evaluates whenever the photos
// list changes (after router.refresh()).
// ============================================================
function useLatestPhotoIdForUrl(
  photos: ProjectPhotoListItem[],
  url: string | null,
): string | null {
  return useMemo(() => {
